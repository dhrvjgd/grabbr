import type { BasePreferences } from "@grabbr/contracts/types";

export const buildBaseArgs = (base: BasePreferences) => {
  const args: string[] = [];
  ...
  if (videoSelection.maxDownloads)
    args.push("--max-downloads", String(videoSelection.maxDownloads));
